...
- the Outlook / Office direct rules that school mail depends on
...
- support school mail by letting you append school-owned mail / SSO / portal domains via `CLASH_DIRECT_EXTRA_DOMAINS`
- send OpenAI related traffic to `PROXY`
- send the rest to `PROXY`
- keep the server IP itself on `DIRECT` to avoid proxy loops

If your school mail also jumps through a campus login page or custom mail host, set `CLASH_DIRECT_EXTRA_DOMAINS` in `server/config/setup.conf`, for example:

```bash
CLASH_DIRECT_EXTRA_DOMAINS=mail.school.edu,login.school.edu,sso.school.edu
```

Those extra domains are added to both the VPS-published subscription and the local example templates as `DIRECT` rules.

If Clash Verge imports a profile and you only see rules like:
...
then you are not using the rule set generated by this project. Refresh the subscription after reinstalling or rerendering on the VPS and confirm the downloaded YAML still contains the Microsoft / school-mail direct rules.
